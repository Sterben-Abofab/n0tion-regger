...
def extract_response_images(body: dict[str, Any]) -> list[ResponseImage]:
    ...
    for item in request_input:
        ...
        for part in content:
            ...
            images.append(decode_response_image(part.get("image_url"), len(images) + 1))
...
async def complete_with_images(
    ...
    *,
    ...
    """Complete one first-turn request using Notion's native attachment flow.
    ...
    attachments = [
        build_attachment(image, await _upload_image(notion, image, thread_id=prep.active_thread_id))
        for image in images
    ]
